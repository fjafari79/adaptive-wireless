#!/usr/bin/env python3
"""
Multi-Parameter Comparison Grid Plot Script
Creates a 2x2 grid of plots comparing four solution methods with different combinations
of vehicle counts and UDP probabilities.

Grid layout:
1. 1m, 3v,  p_u=0.9, p_b=0.2   |  2. 1m, 3v,  p_u=0.9, p_b=0.7
3. 1m, 10v, p_u=0.9, p_b=0.2   |  4. 1m, 10v, p_u=0.9, p_b=0.7
"""
...
def create_subplot(results: Dict, ax: plt.Axes, params: Dict, show_legend: bool = False, 
    ...
    # Add parameters text at the top middle of the plot (use mathcal for V)
    ax.text(0.5, 0.95, f'$|\\mathcal{{V}}| = {params["vehicles"]}, p_{{b}} = {params["p_udp"]:.1f}$',
...
def main():
    ...
    # Fixed parameters
    configurations = [
        {"messages": 1, "vehicles": 3,  "p_tcp": 0.9, "p_udp": 0.2},
        {"messages": 1, "vehicles": 3,  "p_tcp": 0.9, "p_udp": 0.7},
    ...
    for idx in range(1, len(configurations)):
        create_subplot(all_results[idx], axs[idx], configurations[idx], 
    ...
    plt.subplots_adjust(left=0.08, right=0.98, bottom=0.11, top=0.92, wspace=0.1, hspace=0.1)
    ...
    filename_pdf = 'multi_parameter_comparison.pdf'
    ...
    filename_png = 'multi_parameter_comparison.png'
