...
# Add local strategies directory to path for imports
import sys
...
class CommunicationSimulator:
    ...
    def simulate_single_run(self, strategy: Dict, strategy_type: str, p_tcp: float, p_udp: float, 
                           initial_state = "all_ones", max_steps: int = 100) -> Tuple[int, int, bool, int, int, float]:
        """
        ...
        Args:
            initial_state: Either a string pattern ("all_ones", "all_zeros") or a tuple/list 
                          representing the state vector (0 = already has message, 1 = needs message)
        
        Returns: (total_transmissions, steps_taken, success, tcp_transmissions, udp_transmissions, total_time)
        """
        # Initialize state
        if isinstance(initial_state, (tuple, list)):
            # Convert tuple/list state vector to matrix
            current_state_matrix = self.vector_to_matrix(initial_state)
        else:
            # Use string pattern
            current_state_matrix = self.get_initial_state(initial_state)
        current_state_id = self.matrix_to_index(current_state_matrix)
        
        total_transmissions = 0
        tcp_transmissions = 0
        udp_transmissions = 0
        total_time = 0.0  # Track total time with TCP time factor
        steps_taken = 0
        
        while steps_taken < max_steps:
            # Check if terminal state reached
            if self.is_terminal_state(current_state_matrix):
                return total_transmissions, steps_taken, True, tcp_transmissions, udp_transmissions, total_time
            ...
            if strategy_type.lower() == "estimated":
                ...
                try:
                    if action_type == "TCP":
                        ...
                        total_time += transmissions * TCP_TIME_FACTOR  # TCP takes longer
                    elif action_type == "UDP":
                        new_state_matrix, transmissions = self.execute_udp_action(
                            current_state_matrix, msg_id, p_udp
                        )
                        udp_transmissions += transmissions
                        total_time += transmissions * 1.0  # UDP takes base time
                    else:
                        print(f"Unknown action type: {action_type}")
                        return total_transmissions, steps_taken, False, tcp_transmissions, udp_transmissions, total_time
                ...
                if action_str in ["DESTINATION", "UNREACHABLE"]:
                    return total_transmissions, steps_taken, action_str == "DESTINATION", tcp_transmissions, udp_transmissions, total_time
                ...
                try:
                    ...
                    # Track TCP vs UDP transmissions based on action type and apply time factor
            ...
            for p_udp in p_udp_values:
                ...
                if graph_strategy is not None:
                    ...
                    for run in range(num_runs):
                        ...
                        transmissions, steps, success, tcp_trans, udp_trans, total_time = self.simulate_single_run(
    ...
    def _plot_cdf_results(self, results, save_plots=True, plots_dir=None):
        ...
        if save_plots and plots_dir:
            cdf_path = os.path.join(plots_dir, f'transmission_attempts_cdf_m{self.n_messages}_v{self.n_vehicles}.png')
    ...
    def _plot_tcp_udp_cdf_breakdown(self, results, save_plots=True, plots_dir=None):
        ...
        if save_plots and plots_dir:
            cdf_path = os.path.join(plots_dir, f'tcp_udp_breakdown_cdf_m{self.n_messages}_v{self.n_vehicles}.png')
            plt.savefig(cdf_path, dpi=300, bbox_inches='tight')
    ...
    def _save_histogram_plots_separately(self, results: Dict, p_tcp_values: list, p_udp_values: list, plots_dir: str):
        ...
        for sim in results['simulations']:
            ...
            filename = f'histogram_m{self.n_messages}_v{self.n_vehicles}_tcp{p_tcp:.2f}_udp{p_udp:.1f}.png'
    ...
    def _save_cdf_plots_separately(self, results: Dict, p_tcp_values: list, p_udp_values: list, plots_dir: str):
        ...
        for sim in results['simulations']:
            ...
            filename = f'cdf_m{self.n_messages}_v{self.n_vehicles}_tcp{p_tcp:.2f}_udp{p_udp:.1f}.png'
    ...
    def vector_to_matrix(self, state_vector) -> np.ndarray:
        """Convert state vector (tuple/list) to state matrix."""
        state_array = np.array(state_vector, dtype=int)
        if len(state_array) != self.n_messages * self.n_vehicles:
            raise ValueError(f"State vector length {len(state_array)} doesn't match expected {self.n_messages * self.n_vehicles}")
        return state_array.reshape(self.matrix_shape)
